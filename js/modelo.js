// JavaScript Contador de Tempo 

const inicioRelacao = new Date("2015-03-22T19:00:00"); // Altere para a data que vocês começaram
const contadorEl = document.getElementById("contadorTempo");

function atualizarContador() {
    const agora = new Date();
    let diff = agora - inicioRelacao;
    ...
    contadorEl.textContent = `Estamos juntos há: ${anos} anos, ${dias} dias, ${horas}h ${minutos}m ${segundos}s ❤️`;
}

setInterval(atualizarContador, 1000);
atualizarContador();

// Autoplay for some mobile browsers
window.addEventListener("load", () => {
    const audio = document.getElementById("musicaCasal");
    audio.play().catch(() => {
        console.log("Autoplay bloqueado. Clique para ouvir a música.");
    });
});
